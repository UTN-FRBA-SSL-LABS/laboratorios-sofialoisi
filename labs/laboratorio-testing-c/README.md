...
> R: inicializa el carrito dejando su cantidad de productos en 0. Es importante llamarla antes de usar el carrito porque así el programa parte de un estado conocido y evita trabajar con valores basura o datos no inicializados.
...
> R: Linea 24. Esa linea suma solamente el precio unitario del producto y no tiene en cuenta su cantidad.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Multiplicar por la cantidad del producto.
...
> R:Si, las líneas 29 y 30. No se ejecutaron porque ninguno de los tests de cobertura llamo a esa función.
...
> R: Un test unitario prueba una función o parte pequeña del programa de forma aislada. Un test de integración prueba varias funciones trabajando juntas. El bug de carrito_total lo detectó primero un test unitario, cuando se probó el total de un producto con cantidad mayor a 1.
...
> R: En C se puede escribir fuera de los límites de un array y sobrescribir memoria que corresponde a otros datos, lo que puede provocar errores o comportamientos inesperados. En Python y Java existen controles de límites y manejo automático de memoria, por lo que normalmente se genera un error en vez de escribir directamente fuera del array.
...
> R: Los tests permiten comprobar automáticamente si el programa se comporta como se espera. Además, ayudan a detectar errores que pueden no ser evidentes al leer el código y permiten volver a ejecutar las pruebas después de hacer cambios para verificar que no se rompió nada.
...
> R: Porque el bug de carrito_total sumaba solamente el precio unitario. Cuando la cantidad era 1, sumar el precio daba casualmente el mismo resultado que hacer precio × 1. En cambio, con cantidad 2, el resultado correcto debía ser el doble, y ahí el bug quedó en evidencia.
